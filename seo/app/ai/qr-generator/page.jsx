...
export default function QrGenerator() {
  ...
  const [errorCorrectionLevel, setECL] = useState("M");
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        "/api/ai/qr-generate",
        { text, size, margin, errorCorrectionLevel, format, darkColor, lightColor },
      ...
      <div className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div>
            ...
            <input type="range" min={128} max={1024} step={32} value={size} onChange={(e) => setSize(parseInt(e.target.value))} className="w-full" />
          </div>
          <div>
            <p className="text-sm font-medium mb-1">Margin: {margin}</p>
            <input type="range" min={0} max={10} step={1} value={margin} onChange={(e) => setMargin(parseInt(e.target.value))} className="w-full" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="color" value={darkColor} onChange={(e) => setDarkColor(e.target.value)} className="w-full h-10 p-1 rounded-md border" />
          </div>
          <div>
            <p className="text-sm font-medium mb-1">Background</p>
            <input type="color" value={lightColor} onChange={(e) => setLightColor(e.target.value)} className="w-full h-10 p-1 rounded-md border" />
          </div>
        </div>

        <button disabled={loading} onClick={onGenerate} className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#7C3AED] to-[#4C1D95] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer">
          {loading ? <Loader2 className="w-4 h-4 my-1 animate-spin" /> : <QrCode className="w-5" />}
          Generate QR
        </button>
      </div>
        ...
        {!result ? (
          ...
          <div className="mt-3 flex flex-col flex-1 min-h-0">
            <div className="flex-1 overflow-y-auto p-4 bg-purple-50 border border-purple-200 rounded-lg">
              {result.format === "png" ? (
                <img src={result.dataUrl} alt="QR Preview" className="w-full h-auto max-w-full mx-auto" />
              ) : (
                <div className="w-full h-auto max-w-full mx-auto" dangerouslySetInnerHTML={{ __html: result.svg }} />
              )}
            </div>
