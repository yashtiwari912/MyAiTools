  ...
  return (
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
        ...
        <input name="image" type="file" accept="image/*" className="w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-gray-600" required />
      ...
      <div className="w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96">
        <div className="flex items-center justify-between gap-3">
          ...
          {content && (
            <button onClick={copyToClipboard} className="flex items-center gap-1 text-sm px-3 py-1 rounded-md border border-gray-300 hover:bg-gray-100">
        ...
        {!content ? (
          ...
          <textarea readOnly value={content} className="mt-3 w-full flex-1 resize-none p-3 text-sm rounded-md border border-gray-200 outline-none" />
        )}
      </div>
    </div>
  );
